  ...
  return(
        <div href={props.website}  className='' style={{color:props.textColor, display:'inline-block', width:280, height:130, background:'white', borderRadius:20, position:'relative', marginLeft:10, fontFamily:'Roboto'}}>
          <div style={{position:'absolute', width:280, height:130}}>
                <h2 style={{fontSize:'1rem', left:100, top:10, color:'#555', fontSize:'1rem', position: 'absolute', fontFamily:'Roboto', fontWeight:600}}>{props.title}</h2>
                ...
                <div style={{ width:170, right:10, top:35 ,  color:'grey', fontSize:'.52rem', position:'absolute'}}>
                  ...
                  <p style={{width:170, lineHeight:'.9rem', color:'#999', fontSize:'.9rem', maxHeight:28,  whiteSpace: 'normal', textOverflow:'ellipsis',fontFamily:'Roboto', overflow:'hidden', display:'block', fontWeight:100}}> {props.description}
                   </p>
                  </div>
                </div>

                <p  style={{position:'absolute',bottom:20, left:100, color:'#555' , fontSize:'.7rem', margin:0, fontFamily:'Roboto', textTransform:'uppercase', width:100, whiteSpace: 'normal', textAlign:'left'}}>{props.categ}</p>

                <p   style={{position:'absolute',bottom:20, right:10, color:'#555' , fontSize:'.7rem', margin:0, fontFamily:'Roboto', textTransform:'uppercase', width:100, whiteSpace: 'normal', textAlign:'right'}}>{props.price}</p>
